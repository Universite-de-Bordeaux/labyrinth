...
void no_dead_end(const maze_t maze, int x, int y)
{
    ...
    while (x != maze.width - 1 || y != maze.height - 1)
    {
        ...
        // le corps de l'algorithme
        // votre déplacement
        neig[0] = !has_wall_right(maze, x, y) && !get_bool(dead_end, x + 1, y);
        neig[1] = !has_wall_down(maze, x, y) && !get_bool(dead_end, x, y + 1);
        neig[2] = !has_wall_left(maze, x, y) && !get_bool(dead_end, x - 1, y);
        neig[3] = !has_wall_up(maze, x, y) && !get_bool(dead_end, x, y - 1);
        if (neig[0] + neig[1] + neig[2] + neig[3] <= 1)
        {
